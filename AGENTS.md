...
`accessControlled` belongs to the **subject**, not the context. Models put houses, shops and driveways at the pavement edge, which is right for a national route and a design error on a cao tốc — but the same paddy or delta context is correct for a provincial road where the farmhouses really do sit on the verge. What changes is the road. The clause is positive for the usual reason: "no houses beside the road" reads to FLUX as "houses beside the road", so it states where the buildings go instead. It sits immediately after the context in both grammars, since the context is what it corrects. `check:presets` asserts it survives every context in both grammars and stays off the urban subjects.
