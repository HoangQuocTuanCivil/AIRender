 ...
 *
 *   npx tsx scripts/print-prompt.ts <subjectId> <contextId> <lightingId> [describe|instruct]
 */
import { composePrompt, type PromptStyle } from "../src/lib/presets";

const [subject, context, lighting, style = "describe"] = process.argv.slice(2);

if (!subject || !context || !lighting) {
  console.error(
    "usage: print-prompt.ts <subjectId> <contextId> <lightingId> [describe|instruct]",
  );
  process.exit(2);
}

const out = composePrompt(subject, context, lighting, style as PromptStyle);
if (!out) {
